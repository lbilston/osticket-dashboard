<!doctype html>
<?php
// Dashboard Script for OS Ticket
// Created by Luke Bilston (bilston.luke.e@edumail.vic.gov.au)

// Version 0.0.2  Free to use, but may need modifications to suit your enviroment..




include( 'config.php' );
$deptID = 1; //IT Department.  Useful if you use many departments 
				...
				<div class="col-md-12">
					...
					<table width="100%" border="1" class="table">
						<tbody>
							...
							while ( $ticketRow = mysqli_fetch_assoc( $allTicketInfo ) ) 
							{
								$ticketID = $ticketRow[ "ticket_id" ];
								$ticketNumber = $ticketRow[ "number" ];
								$lastUpdate = $ticketRow[ "lastupdate" ];
								$isAnswered = $ticketRow[ "isanswered" ];
								$topicID = $ticketRow[ "topic_id" ];
								...
								/* Topic 4 is custom to Warringa Park School for iOS Application Requests this bypasses all */
								if ( $topicID != 4 )
								{
